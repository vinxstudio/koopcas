<template>
    <div class="transaction-search">
        ...
        <form @submit.prevent="fetchTrans">
            <div class="form-row mb-4 mt-2 align-items-center">
                ...
                <div class="col-sm-2 mb-2">
                    <select class="custom-select custom-select-sm" name="trCode" v-model="trCode" @keyup.enter="fetchTrans()">
                        <option v-for="transType in transTypes" v-bind:value="transType.transTypeId">{{ transType.transTypeDesc }}</option>
                    </select>
                </div>
                <div class="col-sm-2">
                    <input class="form-control form-control-sm" v-model="trYear" name="trYear" placeholder="" type="text" @keyup.enter="fetchTrans()">
                </div>
                <div class="col-sm-2">
                    <input class="form-control form-control-sm" v-model="trCtlNo" name="trCtlNo" placeholder="Control Number" type="text" @keyup.enter="fetchTrans()">
                </div>
                <div class="col-sm-2">
                    <input class="form-control form-control-sm" v-model="trDocNo" name="trDocNo" placeholder="Document Number" type="text" @keyup.enter="fetchTrans()">
                </div>
                <div class="col-sm-2">
                    <input class="form-control form-control-sm" name="explanation" placeholder="Explanation" type="text" @keyup.enter="fetchTrans()">
                </div>
                <div class="col-sm-2">
        ...
        <table class="table table-sm table-bordered table-hover custom-table">
            ...
            <tbody>
                <tr v-for="transsum in transsums" v-bind:key="transsum.id">
                    ...
                    <td><router-link v-bind:to="'/front-office/transaction/'+transsum.trCode+'/'+transsum.trYear+'/'+transsum.trCtlNo+'/'+transsum.trDocNo">View Transaction</router-link></td>
                </tr>
            </tbody>
        </table>
    </div>
