<template>
    <div class="loans-app-search">
        <h1 v-if="this.$route.params.status">Loans Applications</h1>
        <h1 v-else>Loans List</h1>
        ...
        <table class="table table-sm table-bordered table-hover custom-table" v-if="this.$route.params.id">
            ...
            <tbody>
                <tr v-for="loan in loans" v-bind:key="loan.id">
                    ...
                    <td>{{ loan.loanPamt }}</td>
                    <td><router-link v-bind:to="'/loans/loans-master/'+loan.loanBrCode+'/'+loan.loanSlcCode+'/'+loan.loanSltCode+'/'+loan.loanRefNo+'/'+loan.clientIdLoan">View Loan</router-link></td>
                </tr>
            </tbody>
        </table>

        <table class="table table-sm table-bordered table-hover custom-table" v-else-if="this.$route.params.status">
            <thead>
                <tr>
            ...
            <tbody>
                <tr v-for="loanApp in loanApps" v-bind:key="loanApp.id">
                    <td>{{ loanApp.docNo }}</td>
                    <td>{{ loanApp.trDate }}</td>
                    <td>{{ loanApp.clientId }}</td>
                    <td>{{ loanApp.lastName }}, {{loanApp.firstName}} {{loanApp.middleName}}</td>
                    <td>{{ loanApp.loanType }}</td>
                    <td>{{ loanApp.prinAmt }}</td>
                    <td>{{ loanApp.status }}</td>
                    <td>{{ loanApp.apprvBy }}</td>
                    <td>{{ loanApp.ctrlNo }}</td>
                    <td><router-link v-bind:to="'/loans/loans-application-module/'+loanApp.clientId+'/'+loanApp.id+'/loan-app'">View Loan</router-link></td>
                </tr>
            </tbody>
        </table>
        ...
        <table class="table table-sm table-bordered table-hover custom-table" v-else>
            ...
            <tbody>
                <tr v-for="client in clients" v-bind:key="client.id">
                    ...
                    <td><router-link v-bind:to="'/loans/loans-application-module/'+client.clientId+'/'+client.brCode">View Loan</router-link></td>
                </tr>
            </tbody>
        </table>
    </div>
...
export default {
    ...
    mounted() {
        ...
        if(this.status > 0){
            this.fetchLoanApps(this.status);
        }
    },
    ...
    methods: {
        ...
        fetchLoanApps(status){
            fetch(`/api/loanAppsDisplay/${status}`)
                .then(res => res.json())
                .then(res => {
                    this.loanApps = res.data;
