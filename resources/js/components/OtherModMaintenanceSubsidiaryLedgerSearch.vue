<template>
  <div class="other-mod-maintenance-sl-search">
    ...
    >
      ...
      <tbody>
        <tr v-for="osl in osls" v-bind:key="osl.id">
          ...
          <td>
            <router-link
              v-bind:to="'/other-mod/maintenance-subsidiary-ledger/'+osl.oslBrCode+'/'+osl.oslSlcCode+'/'+osl.oslSltCode+'/'+osl.oslRefNo+'/'+osl.clientIdOsl"
    ...
    <table class="table table-sm table-bordered table-hover custom-table" v-else>
      ...
      <tbody>
        <tr v-for="clientosl in clientosls" v-bind:key="clientosl.id">
          ...
          <td>
            <router-link
              v-bind:to="'/other-mod/maintenance-subsidiary-ledger/'+clientosl.clientId"
            >View Account</router-link>
          </td>
        </tr>
      </tbody>
...
export default {
  ...
  methods: {
    ...
    fetchClientOsls(id) {
      fetch(`/api/oslsearch/${id}/maintenance-sl-search`)
        .then(res => res.json())
        .then(res => {
          this.osls = res.data;
...
<style>
.other-mod-maintenance-sl-search {
  width: 100%;
  padding-left: 15px;
  padding-right: 15px;
}
