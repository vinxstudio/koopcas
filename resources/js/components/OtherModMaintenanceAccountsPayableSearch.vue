<template>
  <div class="other-mod-maintenance-ap-search">
    ...
    >
      ...
      <tbody>
        <tr v-for="ap in aps" v-bind:key="ap.id">
          ...
          <td>
            <router-link
              v-bind:to="'/other-mod/maintenance-accounts-payable/'+ap.apBrCode+'/'+ap.apSlcCode+'/'+ap.apSltCode+'/'+ap.apRefNo+'/'+ap.clientIdAp"
    ...
    <table class="table table-sm table-bordered table-hover custom-table" v-else>
      ...
      <tbody>
        <tr v-for="clientap in clientaps" v-bind:key="clientap.id">
          ...
          <td>
            <router-link
              v-bind:to="'/other-mod/maintenance-accounts-payable/'+clientap.clientId"
            >View Account</router-link>
          </td>
        </tr>
      </tbody>
...
export default {
  ...
  methods: {
    ...
    fetchClientAps(id) {
      fetch(`/api/apsearch/${id}/maintenance-ap-search`)
        .then(res => res.json())
        .then(res => {
          this.aps = res.data;
...
<style>
.other-mod-maintenance-ap-search {
  width: 100%;
  padding-left: 15px;
  padding-right: 15px;
}
