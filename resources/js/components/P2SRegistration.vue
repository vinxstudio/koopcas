  ...
  <div class="playtosave">
    ...
    <div class="col-md-16">
      <center>
        <b>Six Digit Entry</b>
        <h1>
          <b>Welcome to 6/46 Play to Save</b>
        </h1>
      ...
      <table class="table table-sm table-bordered custom-table">
        ...
        <tbody>
          <tr v-for="p2sEntry in p2sEntries" v-bind:key="p2sEntry.id">
            ...
            <td>
              <router-link v-bind:to="'/play-to-save/p2s-stabs-list-six/'+p2sEntry.trId+'/'+p2sEntry.clientIdSc">Choose Number</router-link>
            </td>
          </tr>
        </tbody>
      </table>
...
export default {
  ...
  methods:{
    fetchEntries(){
      fetch(`/api/playtosaveregistersix`)
      .then(res => res.json())
      .then(res => {
