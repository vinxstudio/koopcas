  ...
  <div class="loan-master">
    <nav class="navbar navbar-expand-lg p-0" id="toolbar">
      ...
      <ul class="navbar-nav ml-auto">
        <li class="nav-item">
          <router-link
            v-bind:to="'/loans/loans-master/'+client.clientId+'/loan-search-loan'"
          ...
          <router-link to="/loans/loan-search" class="btn btn-sm btn-outline-primary mr-4" v-else>
            <i class="fa fa-search mr-1"></i>
            <span>Find</span>
          </router-link>
          ...
          <button
            class="btn btn-sm btn-outline-primary mr-1"
            type="button"
            v-else-if="this.$route.params.brcode && this.$route.params.slccode && this.$route.params.sltcode && this.$route.params.refno && this.$route.params.clientid"
            @click="insertNew"
          >
            <i class="fa fa-file mr-1"></i>
            <span>
          ...
          <button
            class="btn btn-sm btn-outline-primary mr-1"
            type="button"
            v-if="this.$route.params.brcode && this.$route.params.slccode && this.$route.params.sltcode && this.$route.params.refno && this.$route.params.clientid"
            @click="editLoan(loan)"
          >
                ...
                <div class="col-sm-4 my-1 align-items-end justify-content-end">
                  <fieldset class="my-1 border pb-3 px-3 mx-1">
                    ...
                    <select
                      ...
                      id
                      v-model="loan.loanMemberRating"
                      name="loanMemberRating"
                      disabled
                    >
                      <option disabled>(select one)</option>
                      <option
                        v-for="loanLoanMemberRating in loanLoanMemberRatings"
                        v-bind:value="loanLoanMemberRating.ratingId"
                      >{{ loanLoanMemberRating.ratingDesc }}</option>
                    </select>
                  </fieldset>
                </div>
              </div>
                  ...
                  <div class="form-row">
                    ...
                    <div class="col my-1">
                      <select
                        class="custom-select custom-select-sm"
                        id
                        v-model="loan.loanSltCode"
                        name="loanSltCode"
                        :disabled="selectDisabled"
                      >
                        <option disabled>(select one)</option>
                        <option
                          v-for="loanLoanSlType in loanLoanSlTypes"
                          v-bind:value="loanLoanSlType.slTypeSltCode"
                        >{{ loanLoanSlType.slTypeMDesc }}</option>
                      </select>
                    </div>
                  </div>
                  <div class="form-row">
                    ...
                    <div class="col my-1">
                      <select
                        class="custom-select custom-select-sm"
                        id
                        v-model="loan.loanClass"
                        name="loanClass"
                        :disabled="selectDisabled"
                      >
                        <option disabled>(select one)</option>
                        <option
                          v-for="loanLoanClass in loanLoanClasses"
                          v-bind:value="loanLoanClass.loanClassId"
                        >{{ loanLoanClass.loanClassDesc }}</option>
                      </select>
                    </div>
                  </div>
                  <div class="form-row">
                    ...
                    <div class="col my-1">
                      <select
                        class="custom-select custom-select-sm"
                        id
                        v-model="loan.loanPurpose"
                        name="loanPurpose"
                        :disabled="selectDisabled"
                      >
                        <option disabled>(select one)</option>
                        <option
                          v-for="loanLoanPurpose in loanLoanPurposes"
                          v-bind:value="loanLoanPurpose.loanPurposeId"
                        >{{ loanLoanPurpose.loanPurposeDesc }}</option>
                      </select>
                    </div>
                  </div>
                  <div class="form-row">
                    ...
                    <div class="col my-1">
                      <select
                        class="custom-select custom-select-sm"
                        id
                        v-model="loan.loanIndSection"
                        name="loanIndSection"
                        :disabled="selectDisabled"
                      >
                        <option disabled>(select one)</option>
                        <option
                          v-for="loanIndSection in loanIndSections"
                          v-bind:value="loanIndSection.indSectionId"
                        >{{ loanIndSection.indSectionDesc }}</option>
                      </select>
                    </div>
                  </div>
                  <div class="form-row">
                    ...
                    <div class="col my-1">
                      <select
                        class="custom-select custom-select-sm"
                        id
                        v-model="loan.loanIndustry"
                        name="loanIndustry"
                        :disabled="selectDisabled"
                      >
                        <option disabled>(select one)</option>
                        <option
                          v-for="loanIndDivision in loanIndDivisions"
                          v-bind:value="loanIndDivision.indDivisionId"
                        >{{ loanIndDivision.indDivisionDesc }}</option>
                      </select>
                    </div>
                  </div>
                  <div class="form-row">
                      ...
                      <div class="form-row">
                        ...
                        <div class="col my-1">
                          <select
                            class="custom-select custom-select-sm"
                            id
                            v-model="loan.loanTermPerd"
                            name="loanTermPerd"
                            :disabled="selectDisabled"
                          >
                            <option disabled>(select one)</option>
                            <option
                              v-for="loanLoanTermPerd in loanLoanTermPerds"
                              v-bind:value="loanLoanTermPerd.termId"
                            >{{ loanLoanTermPerd.termDesc }}</option>
                          </select>
                        </div>
                      </div>
                    </div>
                  ...
                  <div class="form-row">
                    ...
                    <div class="col my-1">
                      <select
                        class="custom-select custom-select-sm"
                        id
                        v-model="loan.loanPpmtMode"
                        name="loanPpmtMode"
                        :disabled="selectDisabled"
                      >
                        <option disabled>(select one)</option>
                        <option
                          v-for="loanLoanPaymentMode in loanLoanPaymentModes"
                          v-bind:value="loanLoanPaymentMode.paymentModeId"
                        >{{ loanLoanPaymentMode.paymentModeDesc }}</option>
                      </select>
                    </div>
                  </div>
                  <div class="form-row">
                    ...
                    <div class="col my-1">
                      <select
                        class="custom-select custom-select-sm"
                        id
                        v-model="loan.loanStatus"
                        name="loanStatus"
                        :disabled="selectDisabled"
                      >
                        <option disabled>(select one)</option>
                        <option
                          v-for="loanLoanCtsType in loanLoanCtsTypes"
                          v-bind:value="loanLoanCtsType.ctsTypeCode"
                        >{{ loanLoanCtsType.ctsTypeDesc }}</option>
                      </select>
                    </div>
                  </div>
                </div>
                <div class="col">
                  ...
                  <div class="form-row">
                    ...
                    <div class="col my-1">
                      <select
                        class="custom-select custom-select-sm"
                        id
                        v-model="loan.loanAmorType"
                        name="loanAmorType"
                        :disabled="selectDisabled"
                      >
                        <option disabled>(select one)</option>
                        <option
                          v-for="loanLoanAmorType in loanLoanAmorTypes"
                          v-bind:value="loanLoanAmorType.amorTypeId"
                        >{{ loanLoanAmorType.amorTypeDesc }}</option>
                      </select>
                    </div>
                  </div>
                  <div class="form-row">
                    ...
                    <div class="col my-1">
                      <select
                        class="custom-select custom-select-sm"
                        id
                        v-model="loan.loanDimbFreq"
                        name="loanDimbFreq"
                        :disabled="selectDisabled"
                      >
                        <option disabled>(select one)</option>
                        <option
                          v-for="loanLoanDimFreq in loanLoanDimFreqs"
                          v-bind:value="loanLoanDimFreq.dimFreqId"
                        >{{ loanLoanDimFreq.dimFreqDesc }}</option>
                      </select>
                    </div>
                  </div>
                  <div class="form-row">
                  ...
                  <div class="form-row">
                    ...
                    <div class="col my-1">
                      <input
                        class="form-control form-control-sm"
                        v-model="loan.loanPrepaidPymnts"
                        name="loanPrepaidPymnts"
                        type="number"
                        :readonly="readOnly"
                      >
                    </div>
                ...
                <div class="col-auto">
                  <div class="form-check form-check-sm my-2">
                    <input
                      ...
                      v-model="loan.loanIsExcludeSundays"
                      name="loanIsExcludeSundays"
                      type="checkbox"
                    >
                    <label
                      class="form-check-label"
                    >EXCLUDE SUNDAYS. Extends the Maturity Date and Excludes Sundays in Interest Computation. Applicable only for Straightline Method.</label>
                  </div>
                  <div class="form-check form-check-sm my-2">
                    <input
                      class="form-check-input"
                      v-model="loan.loanIsExcludeHolidays"
                      name="loanIsExcludeHolidays"
                      type="checkbox"
                    >
                    <label
                      class="form-check-label"
                    >EXCLUDE HOLIDAYS. Extends the Maturity Date and Excludes Holidays in Interest Computation. Applicable only for Straightline Method.</label>
                  </div>
                </div>
              </div>
              <div class="form-row justify-content-center">
          ...
          <div class="tab-content">
            <div role="tabpanel" class="tab-pane active" id="duedates">
              <div class="form-row">
                <label
                  class="col-sm-3 col-form-label col-form-label-sm mt-1 text-sm-right text-truncate"
                >Due Date Option:</label>
                <div class="col my-1">
                  <select
                    class="custom-select custom-select-sm"
                    id
                    v-model="loan.loanDdFlag"
                    name="loanDdFlag"
                    :disabled="selectDisabled"
                  >
                    <option disabled>(select one)</option>
                    <option
                      v-for="loanLoanDueDateType in loanLoanDueDateTypes"
                      v-bind:value="loanLoanDueDateType.dueDateTypeID"
                    >{{ loanLoanDueDateType.dueDateTypeDesc }}</option>
                  </select>
                </div>
              </div>
              <div class="form-row">
              ...
              <div class="form-row">
                ...
                <div class="col my-1">
                  <select
                    class="custom-select custom-select-sm"
                    id
                    v-model="loan.loanCollectType"
                    name="loanCollectType"
                    :disabled="selectDisabled"
                  >
                    <option disabled>(select one)</option>
                    <option
                      v-for="loanLoanCollectType in loanLoanCollectTypes"
                      v-bind:value="loanLoanCollectType.collectTypeId"
                    >{{ loanLoanCollectType.collectTypeDesc }}</option>
                  </select>
                </div>
              </div>
              <div class="form-row">
            ...
            <div role="tabpanel" class="tab-pane" id="comakers">
              ...
              <div class="form-row">
                <label
                  class="col-sm-3 col-form-label col-form-label-sm mt-1 text-truncate"
                >Borrower Classification:</label>
                <div class="col my-1">
                  <select
                    class="custom-select custom-select-sm"
                    id
                    v-model="loan.loanMClass"
                    name="loanMClass"
                    :disabled="selectDisabled"
                  >
                    <option disabled>(select one)</option>
                    <option
                      v-for="borrowerClass in borrowerClasses"
                      v-bind:value="borrowerClass.borrowerClassId"
                    >{{ borrowerClass.borrowerClassDesc }}</option>
                  </select>
                </div>
              </div>
              <div class="form-row">
                <label
                  class="col-sm-3 col-form-label col-form-label-sm mt-1 text-truncate"
                >Account Officer In-Charge:</label>
                <div class="col my-1">
                  <input
                    class="form-control form-control-sm"
                    v-model="loan.loanAcctOff"
                    name="loanAcctOff"
                    type="text"
                    :readonly="readOnly"
                  >
              ...
              <div class="form-row">
                ...
                <div class="col my-1">
                  <select
                    class="custom-select custom-select-sm"
                    id
                    v-model="loan.loanSecurity"
                    name="loanSecurity"
                    :disabled="selectDisabled"
                  >
                    <option disabled>(select one)</option>
                    <option
                      v-for="loanLoanSecurity in loanLoanSecurities"
                      v-bind:value="loanLoanSecurity.loanSecurityId"
                    >{{ loanLoanSecurity.loanSecurityDesc }}</option>
                  </select>
                </div>
              </div>
              <div class="form-row">
                <label
                  class="col-sm-3 col-form-label col-form-label-sm mt-1 text-truncate"
                >Type of Collateral:</label>
                <div class="col my-1">
                  <select
                    class="custom-select custom-select-sm"
                    id
                    v-model="loan.loanCollType"
                    name="loanCollType"
                    :disabled="selectDisabled"
                  >
                    <option disabled>(select one)</option>
                    <option
                      v-for="loanLoanCollateralType in loanLoanCollateralTypes"
                      v-bind:value="loanLoanCollateralType.collateralType"
                    >{{ loanLoanCollateralType.collateralTypeDesc }}</option>
                  </select>
                </div>
              </div>
              <div class="form-row">
                <label
                  class="col-sm-3 col-form-label col-form-label-sm mt-1 text-truncate"
                >Appraised Value:</label>
                <div class="col my-1">
                  <input
                    class="form-control form-control-sm"
                    v-model="loan.loanCollAmt"
                    name="loanCollAmt"
                    type="text"
                    :readonly="readOnly"
                  >
              ...
              <div class="form-row">
                ...
                <div class="col my-1">
                  <select
                    class="custom-select custom-select-sm"
                    id
                    v-model="loan.loanBank"
                    name="loanBank"
                    :disabled="selectDisabled"
                  >
                    <option disabled>(select one)</option>
                    <option
                      v-for="loanLoanBankCode in loanLoanBankCodes"
                      v-bind:value="loanLoanBankCode.bcId"
                    >{{ loanLoanBankCode.bcDesc }}</option>
                  </select>
                </div>
              </div>
              <div class="form-row">
  ...
  methods: {
    ...
    deleteLoanById() {
      ...
      if (confirm("Are You Sure?")) {
        fetch(
          `/api/loan/${brcode}/${slccode}/${sltcode}/${refno}/${clientid}`,
          {
            method: "delete"
          }
        )
    ...
    insertNew() {
      this.readOnly = false;
      this.selectDisabled = false;
      this.loan.clientIdLoan = this.$route.params.id;
      this.loan.loanSltCode = "";
      this.loan.loanStatus = "";
      this.loan.loanClass = "";
      this.loan.loanPurpose = "";
