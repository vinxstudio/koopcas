<template>
    <div class="share-capital-search">
        <h1>Deposits List</h1>

        <form @submit.prevent="fetchDeposits">
            <div class="form-row mb-4 mt-2 align-items-center">
                ...
                <div class="col-sm-3 mr-3">
                    <input class="form-control form-control-sm" v-model="search" name="search" placeholder="Enter Client's ID" type="text" @keyup.enter="fetchDeposits()">
                </div>
                <label class="col-form-label col-form-label-sm text-sm-right text-truncate font-weight-bold" for="status">Status</label>
                <div class="col-auto">
                    <button class="btn btn-sm btn-outline-primary mr-1" type="button" @click="fetchDeposits()">
        ...
        <table class="table table-sm table-bordered table-hover custom-table" v-if="this.$route.params.id">
            ...
            <tbody>
                <tr v-for="sc in scs" v-bind:key="sc.id">
                    ...
                    <td>{{ sc.scSlcCode }}</td>
                    <td>{{ sc.scSltCode }}</td>
                    <td>{{ ("00000000"+sc.scRefNo).slice(-8) }}</td>
                    <td>{{ sc.scTrDate }}</td>
                    <td>{{ sc.scMinBal }}</td>
                    <td><router-link v-bind:to="'/deposits/share-capital/'+sc.scBrCode+'/'+sc.scSlcCode+'/'+sc.scSltCode+'/'+sc.scRefNo+'/'+sc.clientIdSc">View Share</router-link></td>
                </tr>
            </tbody>
        </table>
        ...
        <table class="table table-sm table-bordered table-hover custom-table" v-else>
            ...
            <tbody>
                <tr v-for="clientsd in clientsds" v-bind:key="clientsd.id">
                    ...
                    <td>{{ clientsd.accountName }}</td>
                    <td>{{ clientsd.referenceId }}</td>
                    <td><router-link v-bind:to="'/deposits/share-capital/'+clientsd.clientId">View Client</router-link></td>
                </tr>
            </tbody>
        </table>
    </div>
